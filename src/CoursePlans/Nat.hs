...
solveMinimize :: Solver -> [Lit] -> IO () -> Nat -> IO Bool
solveMinimize sat as h n@(Nat xs _) =
  do b <- solve sat as
     if b then
       let mini =
             do v <- modelValueNat sat n
                if v <= giveup then return True else
                  do putStrLn ("Trying " ++ show v ++ " (<= " ++ show m ++ ")...")
                     b <- solve sat ((n .< v):as)
                     if b then do h; mini
                          else return True
        in do h; mini
...
solveMaximize :: Solver -> [Lit] -> IO () -> Nat -> IO Bool
solveMaximize sat as h (Nat xs true) = solveMinimize sat as h (Nat (reverse (map neg xs)) true)
