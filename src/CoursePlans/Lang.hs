...
main =
  do s <- readFile "corpus.txt"
     ...
     putStrLn (show (length ws) ++ " LHS word-groups")

     findCoverings pqs'

cleanup :: [(Phrase,Phrase)] -> [(Phrase,Phrase)]
cleanup pqs =
  [ (p',q')
  | (p,q) <- pqs
  , all (`notElem` englishWords) p
  , let p' = filter isWord . filter (`notElem` smallSwe) $ p
        q' = filter isWord . filter (`notElem` smallEng) $ q
  , not (null p')
  , not (null q')
  ]
 where
  englishWords = words "the are that and small"
  smallEng     = words "the a an one is are in"
  smallSwe     = words "det den en ett är i in"

  isWord s = all isAlpha s
...
groupWords pqs = [ (grp gps p, grp gqs q) | (p,q) <- pqs ]
 where
  ...
  grp []           p = p
  grp (g@(w:_):gs) p
    | w `elem` p     = ("{"++ concat (intersperse ", " g) ++ "}") : grp gs (filter (`notElem` g) p)
    | otherwise      = grp gs p
...
findCoverings1 pqs =
  do putStrLn "=== Finding Greedy Covering ==="
...
findCommonPairs :: [Phrase] -> IO ()
findCommonPairs ps =
  do sequence_ [ putStrLn (v ++ " ~ " ++ w) | (v,w) <- vws ]
     putStrLn (show (length vws) ++ " friend-pairs")
 where
  vws = nub
        [ (v,w)
        | (p,ps') <- ps `zip` tail (iterate tail ps)
        , let p0 = nub p
        , v <- p0
        , w <- p0
        , v < w
        , _:_ <- [ q | q <- ps', v `elem` q, w `elem` q ]
        ]

findCoverings2 :: [(Word,Word)] -> [(Phrase,Phrase)] -> IO ()
findCoverings2 pairs0 pqs =
  do --putStrLn "=== Finding Structure ==="
     --findCommonPairs [ p | (p,q) <- pqs ]
  ...
  do let local a xs =
         ...
         global xs =
           do --putStrLn "-- global round --"
              ...
              -- at least two of the 1s should become a 0
              atLeastTwo sat [ neg y | y <- ys ]
              -- at least one of the 0s should become a 1
              addClause sat [ x | x <- xs, x `notElem` ys ]
              --pullup [ x | x <- xs, x `notElem` ys ]
              if length ys < length xs then global xs else return ()
...
atLeastTwo sat xs =
  do x <- newLit sat
     ...
     atLeast2 sat x x xs
 where
  atLeast2 sat x1 x2 xs =
    do addClause sat (neg x1 : xs)
       if k > 5 then
         do y1 <- newLit sat
            y2 <- newLit sat
            addClause sat [neg y2, y1]
            z1 <- newLit sat
            z2 <- newLit sat
            addClause sat [neg z2, z1]
            addClause sat [neg x2, y2, z2, y1]
            addClause sat [neg x2, y2, z2, z1]
            atLeast2 sat y1 y2 (take k xs)
            atLeast2 sat z1 z2 (drop k xs)
        else
         do sequence_
              [ addClause sat (neg x2 : [ x | x <- xs, x /= y ])
              | y <- xs
              ]
   where
    k = length xs `div` 2
